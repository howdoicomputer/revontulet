...
# Get a list of four satellites currently over Toulouse at a 15 degree
# search radius.
#
...
#
SATS=$(curl -s -X 'GET' \
  "http://0.0.0.0:8000/api/satellites?lat=$TOULOUSE_LAT&lng=$TOULOUSE_LNG&cat=$CAT&alt=$ALT&search_radius=$SEARCH_RAD" \
...
#
curl -s -X 'GET' \
    "http://localhost:8000/api/satellites/above?lat=$TOULOUSE_LAT&lng=$TOULOUSE_LNG&cat=$CAT&alt=$ALT&search_radius=$SEARCH_RAD&sat_color_pairs=$SAT_COLOR_PAIRS" \
...
#
curl -s -X 'GET' \
    "http://localhost:8000/api/satellites/above?lat=$TOULOUSE_LAT&lng=$TOULOUSE_LNG&cat=$CAT&alt=$ALT&search_radius=$SEARCH_RAD&sat_color_pairs=$SAT_COLOR_PAIRS&format=text" \
    -H 'accept: application/json'
